...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifBindMat(-1), unifTransformationMat(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifTransformationMat = context -> glGetUniformLocation(prog, "u_transform");
    unifBindMat = context -> glGetUniformLocation(prog, "u_bind");
...
void ShaderProgram::setTransformationMat(const glm::mat4 t[100]) {
    useMe();

    context->glUniformMatrix4fv(unifTransformationMat, 100, GL_FALSE, &t[0][0][0]);

}

void ShaderProgram::setBindMat(const glm::mat4 b[100]) {
    useMe();

    context->glUniformMatrix4fv(unifBindMat, 100, GL_FALSE, &b[0][0][0]);
}
